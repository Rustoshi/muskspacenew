...
export async function POST(req: Request) {
    try {
        ...
        const fromEmail = (process.env.UNOSEND_FROM_EMAIL || "noreply@muskspace.com").trim();

        if (!unosendApiKey) {
            console.error("UNOSEND_API_KEY is missing from environment variables.");
        ...
        const emailResponse = await fetch("https://www.unosend.co/api/v1/emails", {
            method: "POST",
            headers: {
        ...
        if (!emailResponse.ok) {
            ...
            console.error("Unosend API Error:", emailResponse.status, errorText);
